...
@pytest.mark.asyncio
async def test_allosaurus_setup_lazy_load(mock_read_recognizer, mock_recognizer):
    asr = AllosaurusASR()
...
@pytest.mark.asyncio
async def test_allosaurus_transcribe(mock_read_recognizer, mock_recognizer):
    asr = AllosaurusASR()
...
@pytest.mark.asyncio
async def test_allosaurus_validation_error(mock_read_recognizer):
    asr = AllosaurusASR()
    with pytest.raises(ValidationError):
        await asr.transcribe({}) # Missing path


def test_allosaurus_init_params():
