...
def _check_quality_gate(pre_res: PreprocessorResult) -> None:
    ...
    meta = pre_res.get("meta", {})
    quality = meta.get("audio_quality")
    if quality is None:
        return
    ...
    if not (issues & _BLOCKING_QUALITY_ISSUES):
        return

    msg = quality.get("user_feedback") or "Audio de baja calidad: no se puede transcribir."
    err_code = cast(Optional[str], quality.get("error_code")) or quality_gate_error_code(list(issues))
...
async def _get_raw_ref_tokens(textref: TextRefProvider, text: str, target_ipa: Optional[str], lang: str) -> list[Token]:
    if target_ipa and target_ipa.strip():
        ...
        return raw_ref_tokens
        
    tr_result = await textref.to_ipa(text, lang=lang)
    return tr_result.get("tokens", [])
...
async def _prepare_target_phonemic(
    *,
    ...
    raw_ref_tokens = await _get_raw_ref_tokens(textref, text, target_ipa, lang)
...
def _resolve_oov_handler(pack: Any, oov_handler: Optional[OOVHandlerPort], evaluation_level: RepresentationLevel) -> Optional[OOVHandlerPort]:
    ...
    return OOVHandler(raw_inventory, collapse_threshold=0.3, level=evaluation_level)
...
def _apply_oov_filter_if_needed(
    *,
    ...
    active_oov = _resolve_oov_handler(pack, oov_handler, evaluation_level)
    if active_oov is None:
    ...
    *,
    ...
    try:
        ...
        observed_phonetic, _asr_tokens, _asr_result = await _prepare_observed_phonetic(
            pre=pre, asr=asr, processed_audio=processed_audio,
            pre_audio_res=pre_audio_res, lang=source_lang, norm_params=norm_params,
        )

        target_phonemic, _ref_tokens = await _prepare_target_phonemic(
            pre=pre, textref=textref, text=text, target_ipa=target_ipa,
            lang=target_lang, evaluation_level=evaluation_level, norm_params=norm_params,
        )

        target_repr, observed_repr = _select_representations(
            pack=pack, mode=mode, evaluation_level=evaluation_level,
            observed_phonetic=observed_phonetic, target_phonemic=target_phonemic,
        )

        target_repr, observed_repr = _apply_oov_filter_if_needed(
            pack=pack, oov_handler=oov_handler, evaluation_level=evaluation_level,
            target_repr=target_repr, observed_repr=observed_repr,
        )

        return await _execute_comparison(
            pack=pack, comp=comp, weights=weights, mode=mode,
            evaluation_level=evaluation_level, target_repr=target_repr,
