...
*   **Architecture:** Microkernel / Plug-in Architecture
    ...
    *   **Plugins:** Standard Python packages discovered via `pronunciapa.plugins` entry points.
    *   **Server:** `ipa_server` (Independent FastAPI consumer)
    *   **Plugin Management:**
        *   **Discovery:** `importlib.metadata` (via entry points).
        *   **Lifecycle:** `pip` invoked via `subprocess` for install/uninstall.
