...
## Phase 1: Robust Loading & Defaults
...
- [ ] Task: Implement `load_config(path=None)` in `ipa_core/config/loader.py` with search path logic (Env -> CWD -> Defaults).
...
- [ ] Task: Implement `format_validation_error` utility to transform Pydantic errors into user-friendly strings.
